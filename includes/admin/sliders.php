...
if($task == 'unpublish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."cis_sliders SET `published` = '0' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'publish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."cis_sliders SET `published` = '1' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'delete') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "DELETE FROM ".$wpdb->prefix."cis_sliders WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
$sql = 
		"
			SELECT 
				...
				sp.name,
				sp.published,
				COUNT(sa.id) AS num_items
			FROM ".$wpdb->prefix."cis_sliders sp
			LEFt JOIN ".$wpdb->prefix."cis_images AS sa ON sa.id_slider=sp.id
			WHERE 1 
		";
if($filter_state == 1)
...
?>
<form action="admin.php?page=cis_sliders" method="post" id="wpcis_form">
<div style="overflow: hidden;margin: 0 0 10px 0;">
	<div style="float: left;">
		...
		<a href="admin.php?page=cis_sliders"  class="button">Reset</a>
	</div>
	<div style="float:right;">
		<a href="admin.php?page=cis_sliders&act=new" id="wpcis_add" class="button-primary">New</a>
...
<table class="widefat">
	<thead>
		<tr>
			...
			<th nowrap align="left" style="text-align: left;">Items</th>
			<th nowrap align="left" style="text-align: left;">Shortcode</th>
			<th nowrap align="center" style="width: 30px;text-align: center;">Id</th>
		</tr>
	</thead>
<tbody id="wpcis_sortable" table_name="<?php echo $wpdb->prefix;?>cis_sliders" reorder_type="reorder">
<?php        
			$k = 0;
...
?>
				<tr class="row<?php echo $k; ?> ui-state-default" id="option_li_<?php echo $row->id; ?>">
					...
					<td valign="middle" align="left" style="vertical-align: middle;padding-left: 22px;">
						<a href="admin.php?page=cis_sliders&act=edit&id=<?php echo $row->id;?>"><?php echo $row->name; ?></a>
					</td>
					<td valign="top" align="left" style="vertical-align: middle;">
						<a href="admin.php?page=cis_items&filter_slider=<?php echo $row->id; ?>">Manage Items [Total:<?php echo $row->num_items; ?>]</a>
					</td>
					<td valign="middle" align="left" style="vertical-align: middle;">
						<input class="wpcis_shortcode" value='[creativeslider id=&quot;<?php echo $row->id;?>&quot;]' onclick="this.select()" readonly="readonly" />
					</td>
